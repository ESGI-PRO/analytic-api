...
import { AnalyticsInterface, AnalyticsVisitorsinterface, BrowserInterface,  AnalyticsMousePositionTrackerinterface } from './dto_interface/analytics.interface';
import { CreateAnalyticsDto, CreateAnalyticsVisitorsDto, FindAnalyticsDtoBy, FindAnalyticsVisitorsDto, GetAnalyticEventDto, GetAnalyticsDto, CreateAnalyticsMouseTrackerDto } from './dto_interface/analytics.dto';
...
export class AppService {
    constructor(
      ...
      @InjectModel('AnalyticsMousePositionTracker')
      private analyticsMouseTrackerModel: Model<AnalyticsMousePositionTrackerinterface>,
    ) {}
  ...
  async createMouseTracker(createMouseTrackerDto: CreateAnalyticsMouseTrackerDto): Promise<AnalyticsMousePositionTrackerinterface> {
    const createdMouseTracker = new this.analyticsMouseTrackerModel(createMouseTrackerDto);
    return createdMouseTracker.save();
  }

  //find all mouse tracker
  async findAllMouseTracker(): Promise<AnalyticsMousePositionTrackerinterface[]> {
    return this.analyticsMouseTrackerModel.find().exec();
  }
  //find mouse tracker by params
  async findMouseTrackerByParams(appName: string): Promise<AnalyticsMousePositionTrackerinterface[]> {
